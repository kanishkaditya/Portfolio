...
export default function ReaddListeners(){
...
//       "https://kanishkaditya.github.io/Portfolio/About.html"
//     );
//   }
// }
console.log(button)
...
function cardOpen(block_name, key) {
  ...
  if (projects[key]["link"] != "")
    $(".fa-github").attr(
      "onclick",
      "window.location.href = '" + projects[key]["link"] + "'"
    );
  ...
  $(block_name).css({
    transform:
      ...
      (($(window).width() * window.devicePixelRatio) /
        ...
        ((2 * key + 1) * 15 * rems + (key + 1) * 10 * rems - scroll_value)) +
      "px)",
    "z-index": "12",
  });
...
function cardClose() {
  ...
  $(".fa-github").attr(
    "onclick",
    "window.location.href = 'https://github.com/kanishkaditya'"
  );
  $("body").css({
